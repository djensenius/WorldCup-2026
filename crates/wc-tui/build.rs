//! Build script: generate the `wc26` man page and shell completions.
//!
...
//! path — notably the bash completion is `wc26`, not `wc26.bash`.
...
fn main() {
    ...
    command.set_bin_name("wc26");

    let man = Man::new(command.clone());
    let mut man_page = Vec::new();
    man.render(&mut man_page).expect("render wc26 man page");
    std::fs::write(out_dir.join("wc26.1"), man_page).expect("write wc26.1");

    for (shell, file_name) in [
        (Shell::Bash, "wc26"),
        (Shell::Zsh, "_wc26"),
        (Shell::Fish, "wc26.fish"),
    ] {
        let mut file = File::create(out_dir.join(file_name)).expect("create completion file");
        generate(shell, &mut command, "wc26", &mut file);
    }
}
